...
return new Promise((resolve, reject) => {
  ...
  return new Promise((resolve, reject) => {
    let bankcheckurl ={
      url: `https://api.yuncheapp.cn/pearl-incentive/api/v1/bank/wakeup?did=AE9B50BD-7967-4CE6-80D6-C505521A5E37&kpn=pearl&ve=3.9.0&nt=WIFI&fr=iOS&lon=&kpf=IPHONE&os=14.2&oc=apple&mi=&adve=3.2.0&isp=460_01&sr=1125*2436&lat=&coins=0&ss=&dpbs=3sCt3iAAMjE4NTA5NDI4AQIQAIinM9cFFPd45BAAAAD6YxuA4ai4/iLrMolNAJry&egid=DFP9AF3828A14CFD41466B40EA6B310E595B7341493D2DBA4657060E6CDFED00&md=iPhone%20XS&app=pearl&__clientSign2=ZWuW9l_x97IyMTg1MDk5ODY4YzhkOWU2MDM1OWMzZDY5NjM0MTNkNjc3NzlhNWUxMzY%3D`,
      headers: {
                Cookie: kkdcookie,
                'Connection': 'keep-alive',
                'Content-Type': 'application/json',
     ...
     $.get(bankcheckurl,(error, response, data) =>{
       const result = JSON.parse(data)
         if(logs)  $.log(data)
         message +='📣银行视察\n'
        if(result.message == 'success') {

...
async function signIn() {
    return new Promise((resolve) => {
        let url = {
            url: "https://lrqd.wasair.com/cash/user/advert/sign",
            headers: JSON.parse(xpreadCookie),
            body: {
                "watchAgain": 0,
                "autoTouch": 1
            }, timeout: 60
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (obj.errorCode == 0) {
                    ...
                    console.log('签到失败,原因:' + obj.errorMsg)
                }
            } catch (e) {
                //$.logErr(e, resp);
            } finally {
...
async function sendLogNew() {
    return new Promise((resolve) => {
        let url = {
            url: "https://lrqd.wasair.com/sendLogNew",
            headers: JSON.parse(xpreadCookie),
            body: {
                "topic":"traffic",
                "spot_id":"福利社首页",
...
async function sendLogOur() {
    return new Promise((resolve) => {
        let url = {
            url: "https://lrqd.wasair.com/logs/send/our",
            headers: JSON.parse(xpreadCookie),
            body: {
                "typeBig": "point",
                "typeSmall": "advert",
...
async function getConfig() {
    return new Promise((resolve) => {
        let url = {
            url: "https://lrqd.wasair.com/advert/config/get",
            headers: JSON.parse(xpreadCookie),
            body: {
                "type": "infoFlow"
            }, timeout: 60
...
async function checkPackage() {
    return new Promise((resolve) => {
        let url = {
            url: "https://lrqd.wasair.com/advert/red/package/check/waite",
            headers: JSON.parse(xpreadCookie), timeout: 60
        };
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.errorCode == 0){
                    ...
                    for(let i = 0; i < newslist.length; i++){
                        if(typeof (newslist[i].aid) != "undefined"){
                            ...
                            $.log("开始阅读:"+newsaid+"\n"+newstitle)
                            $.isWait && await $.wait(200*delay)
                            await newsdetail()
                            $.isWait &&await $.wait(800*delay)
                            await newscomplete()
                            if ($.indexIsFinished){
                                console.log('账号['+$.index+']已完成阅读');
                                return true;
                            }
                        }
                    }

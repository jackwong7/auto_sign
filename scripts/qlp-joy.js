...
两个加起来一天一块吧。提现秒到。注册第二天别忘记去提现两块钱。至于红包提现的200大额就不知道了。等到了再说吧。看看有没有什么套路(可多号刷。限越狱用户，一个微信号可以绑定三次游戏。考状元和娶老婆都是这样，这样算下来就是一个微信一天可以提现三块)
...
let tx = $.time("HH:mm") == "10:00" ? 1 : 0  //数字改为1运行脚本可自动提现。建议手动运行。需要提现的时候再改
console.log(tx )
!(async () => {
